...
4. **Archive.** Call `save_briefing_to_repo` with `bodyHtml` set to that HTML (and the header text as `title`). It wraps it in the fixed stylesheet, commits `briefings/briefing-<date>.html`, and returns the GitHub Pages URL.
